...
function M.setup()
  ...
  -- Terminal mode (use <C-\> prefix to avoid leader/space delay)
  map('t', '<C-\\>q', '<C-\\><C-n>:lua require("terminal").close_and_jump()<CR>', 'Close terminal and jump to code')
  map('t', '<C-\\>t', '<C-\\><C-n>:lua require("terminal").toggle()<CR>', 'Toggle terminal off')
  map('t', '<C-\\>]', '<C-\\><C-n>:lua require("terminal").next()<CR>', 'Next terminal')
  map('t', '<C-\\>[', '<C-\\><C-n>:lua require("terminal").prev()<CR>', 'Previous terminal')
  map('t', '<C-\\>c', '<C-\\><C-n>:lua require("terminal").clear_terminal()<CR>i', 'Clear terminal completely')
